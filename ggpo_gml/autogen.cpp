...
dllx double ggpo_do_game_state_save_3_raw(void* _ptr) {
...
dllx double ggpo_idle_raw(void* _ptr) {
	gml_istream _in(_ptr);
	int _arg_timeout_ms;
...
dllx double ggpo_advance_frame_raw(void* _ptr) {
	gml_istream _in(_ptr);
	return ggpo_advance_frame();
}

extern bool ggpo_player_is_active(int player_number);
dllx double ggpo_player_is_active_raw(void* _ptr) {
	gml_istream _in(_ptr);
...
dllx double ggpo_set_input_size_raw(void* _ptr) {
	gml_istream _in(_ptr);
	int _arg_size;
...
dllx double ggpo_start_session_raw(void* _ptr) {
	gml_istream _in(_ptr);
...
dllx double ggpo_start_spectating_raw(void* _ptr, const char* _arg_host_ip) {
	gml_istream _in(_ptr);
	int _arg_num_players;
	_arg_num_players = _in.read<int>();
	uint16_t _arg_local_port;
...
dllx double ggpo_start_synctest_raw(void* _ptr) {
	gml_istream _in(_ptr);
	int _arg_num_players;
	_arg_num_players = _in.read<int>();
	int _arg_frames;
...
dllx double ggpo_close_session_raw(void* _ptr) {
	gml_istream _in(_ptr);
	return ggpo_close_session();
}

extern optional<int> ggpo_add_local_player(int player_num);
dllx double ggpo_add_local_player_raw(void* _ptr) {
	gml_istream _in(_ptr);
	int _arg_player_num;
...
dllx double ggpo_add_remote_player_raw(void* _ptr, const char* _arg_ip) {
	gml_istream _in(_ptr);
	int _arg_player_num;
	_arg_player_num = _in.read<int>();
	uint16_t _arg_port;
...
dllx double ggpo_disconnect_player_raw(void* _ptr) {
	gml_istream _in(_ptr);
...
dllx double ggpo_set_frame_delay_raw(void* _ptr) {
	gml_istream _in(_ptr);
	int _arg_player_handle;
	_arg_player_handle = _in.read<int>();
...
dllx double ggpo_add_local_input_raw(void* _ptr) {
	gml_istream _in(_ptr);
	int _arg_player_handle;
	_arg_player_handle = _in.read<int>();
	gml_buffer _arg_buf;
...
extern int ggpo_synchronize_input(gml_buffer buf);
dllx double ggpo_synchronize_input_raw(void* _ptr) {
	gml_istream _in(_ptr);
	gml_buffer _arg_buf;
	_arg_buf = _in.read_gml_buffer();
