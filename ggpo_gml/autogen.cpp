...
dllx double ggpo_start_spectating_raw(void* _in_ptr, double _in_ptr_size, const char* _arg_host_ip) {
	gml_istream _in(_in_ptr);
	int _arg_num_players;
	_arg_num_players = _in.read<int>();
	uint16_t _arg_local_port;
...
dllx double ggpo_add_remote_player_raw(void* _inout_ptr, double _inout_ptr_size, const char* _arg_ip) {
	gml_istream _in(_inout_ptr);
	int _arg_player_num;
	_arg_player_num = _in.read<int>();
	uint16_t _arg_port;
